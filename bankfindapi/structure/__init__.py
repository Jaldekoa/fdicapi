...
def get_institutions(**kwargs) -> pd.DataFrame:
    ...
    Keyword Args:
        filters (str): The filter for the bank search. **None by default**.
        search (str): Flexible text search against institution records - currently only supporting name search. Search supports text search and fuzzy matching, as opposed to filters that are exact matches. **None by default**.
...
def get_locations(**kwargs) -> pd.DataFrame:
    ...
    Keyword Args:
        filters (str): The filter for the location search. **None by default**.
